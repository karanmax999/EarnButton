  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <a href={`${BLOCK_EXPLORER.BASE}/address/${walletAddress}`} target="_blank" rel="noopener noreferrer"
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: '#111827', border: '1px solid rgba(255,255,255,0.06)' }}>
        {records.map((r, i) => (
          <div key={`${r.txHash}-${i}`}
            className={`flex items-center gap-3 px-5 py-3.5 ${i < records.length - 1 ? 'border-b' : ''}`}
            style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
            {/* Icon */}
            <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full`}
              style={{ background: r.type === 'deposit' ? 'rgba(0,200,150,0.1)' : 'rgba(251,146,60,0.1)' }}>
              {r.type === 'deposit' ? (
                <svg className="h-4 w-4 text-teal-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 10l7-7m0 0l7 7m-7-7v18" />
                </svg>
              ) : (
                <svg className="h-4 w-4 text-orange-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-sm font-semibold text-white capitalize">{r.type}</span>
                <span className="text-xs text-gray-600">{r.vaultName}</span>
              </div>
              <div className="flex items-center gap-2 mt-0.5">
                <a href={r.basescanUrl} target="_blank" rel="noopener noreferrer"
                  className="text-xs font-mono text-teal-500 hover:text-teal-300 transition-colors">
            ...
            {/* Amount */}
            <span className={`text-sm font-bold shrink-0 ${r.type === 'deposit' ? 'text-teal-400' : 'text-orange-400'}`}>
